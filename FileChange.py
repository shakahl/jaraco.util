...
__version__ = '$Rev: 8 $'[6:-2]
__svnauthor__ = '$Author: Jaraco $'[9:-2]
__date__ = '$Date: 17-11-04 17:18 $'[7:-2]
...
class ThreadedNotifier( BlockingNotifier, Thread ):
	...
	>>> notifier = FileChange.ThreadedNotifier( 'c:\\', handler = FileChange.StreamHandler() )
	>>> n.start()
...
class StreamHandler( object ):
	"""
	StreamHandler: a sample handler object for use with the threaded notifier
		that will announce by writing to the supplied stream (stdout by default) the
		name of the file.
	"""
